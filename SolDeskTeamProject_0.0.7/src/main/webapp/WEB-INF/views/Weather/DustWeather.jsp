	...
	<div class="dust-main">
		...
		<div class="dust-center-line">
			...
			<div class="dustCenterContentLeft">
				...
				<div class="dustCenterLeftContentButton">
					>
				</div>
...
$(document).ready(function(){
    ...
    <c:forEach var="i" begin="0" end="${dust.response.body.totalCount - 1}">
        imageUrls.push([
            "${dust.response.body.items.get(i).imageUrl1}",
            "${dust.response.body.items.get(i).imageUrl2}",
            "${dust.response.body.items.get(i).imageUrl3}",
            "${dust.response.body.items.get(i).imageUrl4}",
            "${dust.response.body.items.get(i).imageUrl5}",
            "${dust.response.body.items.get(i).imageUrl6}"
        ]);
    </c:forEach>
    ...
    $('#dustCenterLeftContentButton').click(function(){
        ...
        if (currentIndex >= totalCount) { // 인덱스가 totalCount와 같거나 커지면 다시 0으로 설정
            currentIndex = 0;
        }
    ...
    // 이미지를 표시하는 함수
    function showImage() {
        // 현재 인덱스에 해당하는 이미지 URL 가져오기
        var imageUrlIndex = currentIndex % imageUrls.length;
        var imageUrl = imageUrls[imageUrlIndex];

        // 첫 번째 이미지 URL만 사용하여 이미지 출력
        $('.dustCenterContentMain').empty().append($('<img>').attr('src', imageUrl[0]));
    }
});
</script>
				</div>
