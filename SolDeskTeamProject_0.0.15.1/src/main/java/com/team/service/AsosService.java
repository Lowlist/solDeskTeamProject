...
public class AsosService {
	...
	public void asosDalyInfoApi(String area,SqlData sql,String date) {
		AsosDalyInfo checkCode = new AsosDalyInfo();
		String API_URI = 
		...
		ResponseEntity<AsosDalyInfo> responseCheck = restTemplate.exchange(uri, HttpMethod.GET, entity, AsosDalyInfo.class);
		ResponseEntity<String> response = restTemplate.exchange(uri, HttpMethod.GET, entity, String.class);
		checkCode = responseCheck.getBody();
		if(checkCode.getResponse().getHeader().getResultCode()== "00") {
			sql.setJsonData(response.getBody());
			mapper.insertSqlData(sql);	
		}else {
			System.out.println("응답코드 에러(ASOS)");
		}
