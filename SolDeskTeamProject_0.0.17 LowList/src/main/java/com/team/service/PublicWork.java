...
public class PublicWork {
	...
	public String nowTime() {
        ...
        if (hours > 5) {
            dateTime = LocalDateTime.of(today, fixedTime);
        } else {
            LocalDate yesterday = today.minusDays(1);
            dateTime = LocalDateTime.of(yesterday, fixedTime);
        }

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMddHH");
        nowTime = dateTime.format(formatter);
        return nowTime;
	}
	...
	public String nowDateDust() {
	    ...
	    String nowTime;
        if (hours > 5) {
            nowTime = today.format(formatter);
        } else {
            LocalDate yesterday = today.minusDays(1);
            nowTime = yesterday.format(formatter);
        }
	   	return nowTime;
	}
