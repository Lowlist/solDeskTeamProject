...
console.log("형바주보:"+typeof(WeatherData));
    ...
    if(jsonData.response.header.resultCode != "10"){
        for (let i = 0; i < jsonData.response.body.items.item.length; i++) {
            ...
            if(jsonData.response.body.items.item[i].category == "SKY"){
                if(jsonData.response.body.items.item[i].fcstDate == nowDate()) {
                    if(jsonData.response.body.items.item[i].fcstTime == nowTime()){
                        WeatherData.dataSKY = jsonData.response.body.items.item[i].fcstValue;
                    }
                }
...
function changeInfo(){
    ...
    if(WeatherData.dataSKY=="4"){
        WeatherData.dataSKY="형바주보";
    }
    $('.home-center-left-big-box').text(WeatherData.dataSKY);
    if (clickBoolean) {
...
function cssForMap(){
    ...
    for(i = 0;i<=12;i++){
        $('.dust-area-view').append("<div class=mini-dust-info id=miniDust"+i+"></div>");
    }
    $('.mini-dust-info').css({
